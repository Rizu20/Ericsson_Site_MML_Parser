                ...
                if search_flag:
                    ...
                    if line.startswith("Router=vr_") and 'InterfaceIPv4' in line and 'AddressIPv4' in line:
                        ...
                        if rat_ip_col not in headers:
                            ...
                            ip_last_col_next += 1
                        headers = [cell.value for cell in ws[1]]
                        ws.cell(row=site_count+1,
                                column=headers.index(rat_ip_col)+1).value = line.strip("\n").strip().split(" ")[-1]
                    ...
                    if line.startswith("Router=vr_") and 'RouteTableIPv4Static' in line and 'NextHop' in line:
                        ...
                        headers = [cell.value for cell in ws[1]]
                        ws.cell(row=site_count+1,
                                column=headers.index(gw_ip_col)+1).value = line.strip("\n").strip().split(" ")[-1]
                    ...
                    if line.startswith("VlanPort=") and 'vlanId' in line:
                        ...
                        headers = [cell.value for cell in ws[1]]
                        ws.cell(row=site_count+1,
                                column=headers.index(vlan_col)+1).value = line.strip("\n").strip().split(" ")[-1]
